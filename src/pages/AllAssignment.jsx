...
const AllAssignment = () => {
  ...
  const getData = () => {
    const res = axios.get(
      `http://localhost:5000/api/v1/assignments?page=${size}&size=${limit}`
    );
    return res;
  };
  const { data: assignments, isLoading } = useQuery({
    queryKey: ["assignments" , size, limit],
    queryFn: getData,
  });
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl bg-gray-200 dark:bg-black md:text-5xl text-amber-900 font-bold text-center py-12 ">
        <p
          data-aos="flip-right"
